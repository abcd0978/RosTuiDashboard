...
// RDash 가 ROS 그래프에 남기는 자기 흔적. /rosapi_params 는 ROS2 rosapi 가 파라미터 서비스용으로 따로 띄우는 노드다.
const SELF_NODES = new Set(['/rosbridge_websocket', '/rosapi', '/rosapi_params']);
...
// ── 파라미터 목록 ────────────────────────────────────────────────────────────
// 1 초 그래프 틱과 분리해 5 초마다 따로 갱신한다. 파라미터는 자주 안 바뀌고, ROS2 경로는 노드 수만큼
// 호출이 필요해 매 틱에 얹으면 낭비다.
//
// ROS1: /rosapi/get_param_names 가 마스터의 전역 파라미터를 한 번에 준다(빠르고 정확).
// ROS2: 같은 서비스가 **11 초 걸리고 빈 배열을 준다**(실측). 못 쓴다. 대신 노드마다
//       <node>/list_parameters(rcl_interfaces/srv/ListParameters)를 부른다 — 노드당 10~20ms.
//       이름은 rosapi 의 ROS2 관례대로 "노드:파라미터"(/turtlesim:background_r)로 합친다.
async function refreshParams() {
  if (!rb || !rb.ready) return;
  if (VER !== '2') {
    const o = await rb.call('/rosapi/get_param_names');
    if (o && o.names) cacheParams = o.names;
    return;
  }
  // 노드 목록을 스스로 구한다 — cacheNodes 는 "클라이언트가 붙어야 도는" 그래프 틱이 채우므로,
  // 여기서 그걸 기다리면 첫 클라이언트가 붙기 전까진 파라미터가 영영 안 채워진다(실제로 그랬다).
  let nodes = cacheNodes.filter((n) => !SELF_NODES.has(n));   // RDash 자신의 파라미터는 그래프가 아니다
  if (!nodes.length) {
    const nr = await rb.call('/rosapi/nodes');
    nodes = [...new Set(((nr && nr.nodes) || []))].filter((n) => !SELF_NODES.has(n));
  }
  if (!nodes.length) return;
  const res = await Promise.all(nodes.map((n) => rb.call(`${n}/list_parameters`, { prefixes: [], depth: 0 })));
  const out = [];
  nodes.forEach((n, i) => {
    const names = res[i] && res[i].result && res[i].result.names;
    if (names) for (const p of names) out.push(`${n}:${p}`);
  });
  if (out.length) cacheParams = out;   // 전부 실패했으면 직전 목록 유지
}
const kickParams = () => { refreshParams().catch(() => {}); };
setTimeout(kickParams, 1500);   // 5 초를 기다리지 않고 첫 목록을 바로 채운다
setInterval(kickParams, 5000);
...
export function rbTelemetryCore(send) {
  rbEnsure();
  if (!telemClients.size) kickParams();   // 첫 클라이언트 — 5 초 주기를 기다리지 않고 파라미터를 바로 채운다
