...
import { mkScene } from './views/scene3d.js';
import { buildGraph } from './views/graph.js';
...
function removeLeaf(id) {
  const p = findParent(tree, id, null);
  if (!p) { tree = leaf('graph'); focusId = tree.id; render(); return; }   // 루트면 리셋
...
function splitLeafInto(id, dir, type) {
  const p = findParent(tree, id, null), target = leafById(tree, id), nl = leaf(type);
  const repl = { split: dir, ratio: 0.5, a: target, b: nl };
  if (!p) tree = repl;
  else if (p.a === target) p.a = repl;
  else p.b = repl;
  focusId = nl.id;
...
function render() {
  for (const [, inst] of live) { try { inst.dispose && inst.dispose(); } catch (_) { /* */ } }
  live.clear();
  root.innerHTML = '';
...
function renderNode(node, container) {
  if (node.split) {
    const box = el('div', { class: 'wssplit ' + node.split });
    container.append(box);
    const pa = el('div', { class: 'wspane' }), pb = el('div', { class: 'wspane' });
    pa.style.flex = node.ratio + ' 1 0';
    pb.style.flex = (1 - node.ratio) + ' 1 0';
    const divi = el('div', { class: 'wsdiv ' + node.split });
    box.append(pa, divi, pb);
    renderNode(node.a, pa);
    renderNode(node.b, pb);
...
function dividerDrag(divi, node, box, pa, pb) {
  divi.onmousedown = (e) => {
    ...
    const move = (ev) => {
      ...
      node.ratio = ratio;
      pa.style.flex = ratio + ' 1 0';
      pb.style.flex = (1 - ratio) + ' 1 0';
    };
...
function renderLeaf(node, container) {
  ...
  typeSel.value = node.panel;
  typeSel.onchange = () => setType(node.id, typeSel.value);
  ...
  let inst = {};
  try { inst = P.make(body, node.cfg, ctx, P.arg) || {}; } catch (err) { body.append(emptyState('⚠', '패널 오류', String(err && err.message || err))); }
  live.set(node.id, inst);
}

// ── 공통: 토픽 셀렉트 ──
function topicSelect(ctx, cfg, filter, label, key, onChange) {
  const s = el('select', { class: 'wssel' });
  s.append(el('option', { value: '' }, '(' + label + ')'));
  state.items.filter((i) => i.kind === 'topic' && !(i.name || '').includes('/_action/') && filter(i)).forEach((i) => s.append(el('option', { value: i.name }, i.name)));
  if (cfg[key]) s.value = cfg[key];
  s.onchange = () => { cfg[key] = s.value; ctx.save(); onChange(s.value); };
  ctx.cfgSlot.append(s);
  return s;
}
...
function makeRaw(host, cfg, ctx) {
  const pre = el('pre', { class: 'wsraw' });
  host.append(pre);
  let es = null;
  const sub = (t) => {
    if (es) { es.close(); es = null; }
    pre.textContent = '';
    if (!t) { pre.append(emptyState('🧾', '토픽 선택', '헤더에서 토픽을 고르세요')); return; }
    es = openStream('/echo?topic=' + encodeURIComponent(t), (d) => { try { pre.textContent = JSON.parse(d); } catch (_) { /* */ } });
  };
  topicSelect(ctx, cfg, () => true, '토픽', 'topic', sub);
  sub(cfg.topic);
...
function makePlot(host, cfg, ctx) {
  ...
  const cv = el('canvas', { class: 'wscanvas' }), legend = el('div', { class: 'wslegend' });
  host.append(cv, legend);
  const series = {}, order = [];
  let t0 = Date.now(), es = null, raf = 0, alive = true;
  const sub = (t) => {
    if (es) { es.close(); es = null; }
    ...
    legend.innerHTML = '';
    if (!t) return;
    es = openStream('/echo?topic=' + encodeURIComponent(t), (d) => {
      let txt;
      try { txt = JSON.parse(d); } catch (_) { return; }
      const nums = numeric(txt), tt = (Date.now() - t0) / 1000;
  ...
  const drawLegend = () => {
    ...
    order.forEach((k, i) => {
      ...
      chip.onclick = () => { cfg.fields[k] = !on; ctx.save(); drawLegend(); };
      legend.append(chip);
    });
  };
  const draw = () => {
    const W = cv.width = cv.clientWidth, H = cv.height = cv.clientHeight, ctx2 = cv.getContext('2d');
    ctx2.clearRect(0, 0, W, H);
    const keys = order.filter((k) => cfg.fields[k] !== false && series[k] && series[k].length > 1);
    if (!keys.length) return;
    let mn = Infinity, mx = -Infinity, t1 = Infinity, t2 = -Infinity;
    for (const k of keys) for (const [t, v] of series[k]) { if (v < mn) mn = v; if (v > mx) mx = v; if (t < t1) t1 = t; if (t > t2) t2 = t; }
    if (mx - mn < 1e-9) { mx += 1; mn -= 1; }
    const cs = getComputedStyle(document.documentElement), grid = cs.getPropertyValue('--line') || '#232b36';
    ctx2.strokeStyle = grid;
    ctx2.lineWidth = 1;
    for (let i = 0; i <= 3; i++) { const y = 4 + (H - 8) * i / 3; ctx2.beginPath(); ctx2.moveTo(0, y); ctx2.lineTo(W, y); ctx2.stroke(); }
    keys.forEach((k) => {
      const ci = order.indexOf(k);
      ctx2.strokeStyle = PAL[ci % PAL.length];
      ctx2.lineWidth = 1.4;
      ctx2.beginPath();
      series[k].forEach(([t, v], i) => { const x = (t - t1) / (t2 - t1 || 1) * (W - 6) + 3, y = H - 6 - (v - mn) / (mx - mn) * (H - 14); i ? ctx2.lineTo(x, y) : ctx2.moveTo(x, y); });
      ctx2.stroke();
    });
  };
...
function makeStream(host, cfg, ctx, arg) {
  const tbl = el('div', { class: 'wsstream' });
  host.append(tbl);
  const rows = [];
  ...
  const s = openStream(arg.ep, (d) => {
    let blk;
    try { blk = JSON.parse(d); } catch (_) { return; }
    if (arg.kind === 'log') {
      const L = ['DEBUG', 'INFO', 'WARN', 'ERROR', 'FATAL'];
      const lv = /level:\s*(\d+)/.exec(blk), nm = /name:\s*["']?([^\n"']+)/.exec(blk), ms = /msg:\s*["']?(.*)/.exec(blk), l = lv ? +lv[1] : 0;
      const idx = l >= 50 ? 4 : l >= 40 ? 3 : l >= 30 ? 2 : l >= 20 ? 1 : 0;
      push(L[idx], (nm ? nm[1].trim() : '?') + ': ' + (ms ? ms[1].replace(/["']\s*$/, '').trim() : ''), idx >= 3 ? 'ERROR' : idx === 2 ? 'WARN' : 'OK');
    } else {
      const LV = ['OK', 'WARN', 'ERROR', 'STALE'];
      const si = blk.indexOf('status:'), sb = si >= 0 ? blk.slice(si) : blk;
      for (const part of sb.split(/\n\s*- /).slice(1)) {
        const lv = /level:\s*(\d+)/.exec(part), nm = /name:\s*["']?(.*)/.exec(part), ms = /message:\s*["']?(.*)/.exec(part), l = lv ? +lv[1] : 0;
        push(LV[l] || '?', (nm ? nm[1].replace(/["']\s*$/, '').trim() : '?') + ': ' + (ms ? ms[1].replace(/["']\s*$/, '').trim() : ''), LV[l] || 'OK');
      }
    }
  });
...
function makeImage(host, cfg, ctx) {
  const img = el('img', { class: 'wsimg' }), ov = el('canvas', { class: 'wsov' });
  const stage = el('div', { class: 'wsstage' }, img, ov);
  host.append(stage);
  ...
  const drawOv = () => {
    ...
    const iw = (cam && cam.width) || img.naturalWidth || cw, ih = (cam && cam.height) || img.naturalHeight || ch, kx = cw / iw, ky = ch / ih, c = ov.getContext('2d');
    ...
    for (const b of ann.boxes) {
      ...
      c.strokeStyle = '#6fd08c';
      c.strokeRect(x, y, w, h);
      const tag = (b.label || 'obj') + (b.score ? ' ' + (b.score * 100 | 0) + '%' : '');
      c.fillStyle = '#6fd08c';
      const tw = c.measureText(tag).width + 5;
      c.fillRect(x, y - 13, tw, 13);
      c.fillStyle = '#0d1116';
      c.fillText(tag, x + 3, y - 2);
    }
    ...
    c.fillStyle = '#e2c85a';
    for (const t of ann.texts) c.fillText(t.t, t.x * kx, t.y * ky);
...
function makeScene(host, cfg, ctx) {
  const cv = el('canvas', { class: 'wsgl' }), labelDiv = el('div', { class: 'wsgllabels' });
  const stage = el('div', { class: 'wsstage', style: 'height:100%' }, cv, labelDiv);
  host.append(stage);
  const scene = mkScene(cv, labelDiv, { textContent: '' });
  let cloudES = null, markerES = null, tfES = null;
  ...
  const tf = el('label', { class: 'wschk' }, (() => { const c = el('input', { type: 'checkbox' }); c.checked = cfg.tf !== false; c.onchange = () => { cfg.tf = c.checked; ctx.save(); subTF(c.checked); }; return c; })(), 'TF');
  ctx.cfgSlot.append(tf);
  [['T', 'top'], ['F', 'front'], ['S', 'side'], ['◆', 'iso']].forEach(([t, p]) => ctx.cfgSlot.append(el('button', { class: 'wsib', title: '뷰: ' + p, onclick: () => scene.view(p) }, t)));
  subCloud(cfg.topic);
  subMarker(cfg.marker);
  subTF(cfg.tf !== false);
...
function makeGraph(host, cfg, ctx) {
  ...
  svg.setAttribute('class', 'wsgraph');
  host.append(svg);
  const P = new Map();
  let alive = true, raf = 0, iterTimer = 0;
  function sim() {
    const g = buildGraph(), W = host.clientWidth || 400, H = host.clientHeight || 300, ids = [...g.ents.keys()];
    ...
    for (let it = 0; it < 90; it++) {
      for (const a of ids) {
        ...
        fx += (W / 2 - pa.x) * 0.012;
        fy += (H / 2 - pa.y) * 0.012;
        pa.x += Math.max(-6, Math.min(6, fx));
        pa.y += Math.max(-6, Math.min(6, fy));
      }
  ...
  function draw(g, W, H) {
    ...
    const cyan = cs.getPropertyValue('--cyan').trim() || '#57c7d4', mag = cs.getPropertyValue('--mag').trim() || '#c78ad2', blue = cs.getPropertyValue('--blue').trim() || '#6f9be0', fg = cs.getPropertyValue('--fg').trim() || '#d5dae2';
    for (const e of g.edges) {
      ...
      ln.setAttribute('stroke', e.kind === 'service' ? blue : e.kind === 'action' ? mag : '#5a6675');
      ln.setAttribute('stroke-width', '1');
      if (e.kind === 'action') ln.setAttribute('stroke-dasharray', '3 3');
      svg.append(ln);
    }
    for (const [id, ent] of g.ents) {
      const p = P.get(id);
      if (!p) continue;
      const isNode = ent.type === 'node';
      const c = document.createElementNS(NS, 'circle');
      c.setAttribute('cx', p.x);
      c.setAttribute('cy', p.y);
      c.setAttribute('r', isNode ? 5 : 3.5);
      c.setAttribute('fill', isNode ? cyan : ent.type === 'service' ? blue : ent.type === 'action' ? mag : '#3f5266');
      svg.append(c);
      const tx = document.createElementNS(NS, 'text');
      tx.setAttribute('x', p.x + 7);
      tx.setAttribute('y', p.y + 3);
      tx.setAttribute('font-size', '9');
      tx.setAttribute('fill', isNode ? fg : '#8b97a7');
      tx.textContent = id.replace(/^\//, '');
      svg.append(tx);
    }
  }
  sim();
  iterTimer = setInterval(() => { if (alive) sim(); }, 2500);
...
function open() {
  ...
  if (!root) {
    ...
    root = el('div', { class: 'wsroot' });
    host.append(bar, root);
    document.body.append(host);
  }
  tree = load();
  focusId = firstLeaf(tree).id;
  render();
  document.getElementById('ws').classList.add('on');
}

function close() {
  const h = document.getElementById('ws');
...
function mkAddMenu() {
  const wrap = el('div', { style: 'position:relative' }), btn = el('button', {}, '＋ 패널');
  ...
  for (const k in PANELS) {
    const it = el('div', { style: 'padding:4px 8px;cursor:pointer;border-radius:4px;font-size:12px', onmouseover: (e) => e.target.style.background = 'var(--hover)', onmouseout: (e) => e.target.style.background = '' }, PANELS[k].icon + ' ' + PANELS[k].name);
    it.onclick = () => { menu.style.display = 'none'; addPanel(k); };
    menu.append(it);
  }
