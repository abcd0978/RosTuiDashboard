...
import { pad, padL, sparkline, LEFT_W } from '../../../../shared/util.js';
...
export function TreePanel() {
  ...
  const treeRows = win.map((r, i) => {
    ...
    const mark = !it ? '' : (isTopic ? (live ? '●' : stale ? '⚠' : '·') : { param: 'P', service: 'S', node: 'N', action: 'A' }[kind] || '·');
    ...
    const kindColor = { param: 'magenta', service: 'blue', node: 'green', action: 'cyan' }[kind];
    return h(Box, { key: i },
      h(Text, {
