...
| `/api/proto` | `name`, `type`, `kind`=`topic`\|`service` (default `topic`) | `{"yaml": "{linear: {x: 0}, …}", "skel": {…}\|null, "type": "geometry_msgs/Twist"}` — publish/call-form prefill. `kind=service` builds the **service request** skeleton instead; a service item in the graph snapshot has no `ty`, so omitting `type` there is normal — the backend resolves it from `name`. `yaml` is a one-line flow-YAML you can drop straight into a text box. `skel` is the raw message/request skeleton (nested object of default values) for clients that render a **field-by-field** form instead. Always returns something (`yaml:"{}"`, `skel:null` if it can't build one). Also `presets` (`[{values:{"field.path":v}, yaml}]`) and `reply` (topic name or null) from bundled `shared/pubdefaults.json` + per-machine `~/.rdash_pubdefaults.json` (same topic → home wins) — see `shared/pubdefaults.js`. |
