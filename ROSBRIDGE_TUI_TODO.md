...
**Web(`web/server.js`)은 rosbridge를 띄워 사용**하는 구조다.
...
- `src/store.js` 는 모든 ROS 동작을 **명령 문자열 → `spawnJob`/`runOnce`** 로 실행한다
  (`be.publish(...)` 가 반환하는 `rostopic pub -1 ...` 같은 문자열을 자식 프로세스로 띄움).
- `RDASH_BACKEND=rosbridge` 로 바꿔도 `RosbridgeBackend`(backend.js)는 `CliBackend` 를 상속해
...
- 실제 rosbridge 클라이언트 연동(`RosbridgeClient` 사용: advertise/publish/subscribe/call_service)은
  **`web/server.js` 에만** 있다(`rbEnsure`/`rbEcho`/`rbTelemetryCore`/`rbPublish`). TUI 에는 없다.

## 무엇을 해야 하나

TUI 의 ROS 동작을 `be.kind === 'rosbridge'` 일 때 CLI 대신 `RosbridgeClient`(src/lib/rosbridge.js)
...
- **텔레메트리/그래프**: 현재 `python3 - telemetry.py` 스트림 → rosbridge 면 `/rosapi/topics`
  `/rosapi/nodes` + `subscribe` 로 Hz 카운트(server.js `rbTelemetryCore` 이식).
...
- **teleop**: server.js 처럼 `rb.publish` 를 10Hz `setInterval` 로(재spawn 없이).
- **로컬 전용 기능**(node 리소스 CPU/RSS, rosbag 녹화/재생, 프로세스킬 등)은 원격 rosbridge 에선
  의미가 없으니 rosbridge 모드에서 비활성/미지원 처리(`RosbridgeBackend` 주석 참고).

## 참고 / 재사용 포인트

- `web/server.js` 의 rosbridge 분기 전부가 이식 템플릿이다(같은 `RosbridgeClient` API).
- `src/lib/rosbridge.js`: `subscribe`/`call`/`publish`(publish 는 최초 1회 auto-advertise).
