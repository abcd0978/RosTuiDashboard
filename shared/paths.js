...
export const ECHO_MUX = join(PY, 'stream', 'ros_echo_mux.py');   // 단일 rclpy 노드 echo 멀티플렉서(프로세스 폭증 해결)
...
// 텔레메트리는 파일 경로가 아니라 내용을 stdin 으로 넘겨 실행한다(원격 셸에서도 파일 없이 돌게).
export const TELEM = readFileSync(join(PY, 'telemetry', 'telemetry.py'), 'utf8');
export const TELEM2 = readFileSync(join(PY, 'telemetry', 'telemetry_ros2.py'), 'utf8');
