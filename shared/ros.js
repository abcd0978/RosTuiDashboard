...
import { sourcePrelude } from './overlays.js';
...
export function rosSpawn(inner, env, detached) {
  ...
  // sourcePrelude(): 비로그인 셸이라 백엔드를 띄운 셸이 오버레이를 소싱했는지와 무관하게 매번
  // distro+오버레이를 직접 소싱한다 — 그래야 custom action/msg 타입을 어떤 launching env 에서도 쓴다.
  ...
  return spawn('bash', ['-c', sourcePrelude() + inner], opts);
}
